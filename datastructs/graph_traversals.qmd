...
BFS for a graph is similar to BFS for a tree. The main difference is that graphs can contain cycles, so we may visit the same vertex again. To avoid processing a vertex more than once, we use a boolean array to mark the visited vertices.
